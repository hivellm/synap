# Spec: C# SDK RPC transport on Thunder

## MODIFIED Requirements

### Requirement: RPC transport implementation
The C# SDK's RPC transport SHALL be implemented on `HiveLLM.Thunder`; the SDK MUST
NOT contain its own framing, MessagePack codec or reconnect logic.

#### Scenario: Commands still round-trip
Given a client in RPC mode against a running server
When it issues `SetAsync` followed by `GetAsync`
...
Then it throws a protocol exception without allocating the claimed size

## ADDED Requirements

### Requirement: No typeless deserialization
The SDK SHALL NOT use `MessagePack.Typeless` or any type-resolving deserializer on
wire data.

#### Scenario: Source audit
Given the `Synap.SDK` sources
When they are searched for `Typeless`
