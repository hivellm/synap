...
- [x] 1.5 Close PRs #222-#229 as superseded (each with rationale comment) and delete their dependabot branches on origin; verified zero dependabot branches and zero open PRs remain
- [x] 1.6 Set [workspace.package] version = "1.0.0-rc.1" in root Cargo.toml (subsequently cut to 1.0.0)
...
- [x] 2.1 Update or create documentation covering the implementation (CHANGELOG [1.0.0] "Changed": version baseline + sysinfo/mlua/rmcp/TS-SDK dep bumps)
- [x] 2.2 Write tests covering the new behavior (spec invariant verified: `grep -L version.workspace crates/*/Cargo.toml sdks/rust/Cargo.toml` returns nothing — all members inherit; full suite green under bumped deps)
