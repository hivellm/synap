...
- [ ] 1.2 Emit an explicit drop/lag signal when stream retention forces a drop
- [ ] 1.3 Index stream consume by offset (index = offset - min_offset) for O(1) seek
- [ ] 1.4 Model queue consumers explicitly with a per-consumer prefetch window
- [ ] 1.5 Implement fair round-robin dispatch across queue consumers and report the real consumer count
- [ ] 1.6 Replace the global 1s ACK-deadline sweep with a per-queue deadline heap/timer-wheel
