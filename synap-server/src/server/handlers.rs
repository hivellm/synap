...
async fn handle_command(state: AppState, request: Request) -> Result<Response, SynapError> {
    ...
    let result = match request.command.as_str() {
        ...
        "list.range" => handle_list_lrange_cmd(&state, &request).await, // Alias for SDK compatibility
        "list.llen" => handle_list_llen_cmd(&state, &request).await,
        "list.len" => handle_list_llen_cmd(&state, &request).await, // Alias for SDK compatibility
        "list.lindex" => handle_list_lindex_cmd(&state, &request).await,
        "list.index" => handle_list_lindex_cmd(&state, &request).await, // Alias for SDK compatibility
        "list.lset" => handle_list_lset_cmd(&state, &request).await,
        "list.set" => handle_list_lset_cmd(&state, &request).await, // Alias for SDK compatibility
        "list.ltrim" => handle_list_ltrim_cmd(&state, &request).await,
        "list.trim" => handle_list_ltrim_cmd(&state, &request).await, // Alias for SDK compatibility
        ...
        "set.card" => handle_set_size_cmd(&state, &request).await, // Alias for SDK compatibility (Redis-style)
        ...
        "set.interstore" => handle_set_inter_cmd(&state, &request).await, // Alias for SDK compatibility (note: returns result, doesn't store)
