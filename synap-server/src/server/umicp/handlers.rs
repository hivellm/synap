...
//! Updated for UMICP v0.2.3 - Native JSON types support
...
pub async fn handle_umicp_request(
    state: UmicpState,
    envelope: Envelope,
) -> Result<Envelope, SynapError> {
    debug!("📦 Processing UMICP envelope: {:?}", envelope.operation());

    // Extract capabilities
    let caps = envelope
        .capabilities()
        .ok_or_else(|| SynapError::InvalidRequest("Missing capabilities".to_string()))?;
    ...
    let operation = caps
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            SynapError::InvalidRequest("Missing 'operation' in capabilities".to_string())
        })?;
    ...
    let mcp_result =
        crate::server::mcp_handlers::handle_mcp_tool(mcp_request, state.app_state.clone()).await;
...
/// Convert UMICP capabilities to MCP CallToolRequest
/// Now with native JSON types support (v0.2.3)
...
/// Now with native JSON types (v0.2.3)
fn create_success_response(request: Envelope, result: Value) -> Result<Envelope, SynapError> {
    ...
    let response = Envelope::builder()
        ...
        .build()
        .map_err(|e| SynapError::InternalError(format!("Failed to build envelope: {}", e)))?;
...
/// Now with native JSON types (v0.2.3)
fn create_error_response(request: Envelope, error_message: &str) -> Result<Envelope, SynapError> {
    let mut response_caps = HashMap::new();
