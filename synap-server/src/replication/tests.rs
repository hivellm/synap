...
use crate::replication::types::ReplicationOperation;
use std::sync::Arc;
...
async fn test_master_replica_sync() {
    ...
    master_kv.set("key1", b"value1".to_vec(), None).await.unwrap();
    master_kv.set("key2", b"value2".to_vec(), None).await.unwrap();
...
async fn test_replica_initialization() {
    ...
    let replica = ReplicaNode::new(replica_config, replica_kv)
        .await
        .unwrap();
