...
async fn test_hybrid_storage_performance_characteristics() {
    ...
    println!("  Insert: {:?} ({:.2} µs/key)", insert_duration, insert_duration.as_micros() as f64 / 1000.0);
    println!("  Read: {:?} ({:.2} µs/key)", read_duration, read_duration.as_micros() as f64 / 1000.0);
