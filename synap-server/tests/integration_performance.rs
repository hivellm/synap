...
async fn test_compact_stored_value_persistence() {
    ...
    store.set("persistent_key", b"persistent_value".to_vec(), None)
        .await
        .unwrap();
    
    // Set expiring value
    store.set("expiring_key", b"expiring_value".to_vec(), Some(3600))
        .await
        .unwrap();
    ...
    for i in 0..num_tasks {
        ...
        let handle = tokio::spawn(async move {
            for j in 0..ops_per_task {
                ...
                store_clone.set(&key, vec![i as u8, j as u8], None)
                    .await
                    .unwrap();
            }
        });
